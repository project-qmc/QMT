...
class TabMain:
    ...
    def displayMNStatus(self, currMN):
        ...
        for mn in self.all_masternodes.get('masternodes'):
            ...
            if mn.get('txhash') == currMN['collateral'].get('txid') and mn.get('outidx') == currMN['collateral'].get(
                    'txidn'):
                statusData = mn
        ...
        self.ui.btn_details[masternode_alias].clicked.connect(
            lambda: self.onDisplayStatusDetails(masternode_alias, statusData))
        ...
        if statusData is None:
            ...
            if statusData['balance'] is not None:
                self.ui.mnBalance[masternode_alias].setText(
                    '&nbsp;<span style="color:purple">%s QMC</span>' % str(statusData['balance']))
    ...
    def onRemoveMN(self, data=None):
        if not data:
            ...
            reply = self.caller.myPopUp(QMessageBox.Warning, 'Confirm REMOVE',
                                        "Are you sure you want to remove\nmasternoode:'%s'" % masternode_alias,
                                        QMessageBox.No)
    ...
    def onStartAllMN(self):
        ...
        # Check RPC & dongle
        if not self.caller.rpcConnected:
            self.caller.myPopUp2(QMessageBox.Critical, 'QMT - hw/rpc device check',
    ...
    def sendBroadcast(self, text):
        ...
        if json.dumps(ret2)[1:26] == "Masternode broadcast sent":
            ...
            message += "the output of the <b>./qmc-cli masternode status</b> command on the VPS should show:<br>"
    ...
    def startMN(self):
        if self.caller.hwStatus != 2:
            self.caller.myPopUp2(QMessageBox.Question, 'QMT - hw device check', self.caller.hwStatusMess,
                                 QMessageBox.Ok)
        elif not self.caller.rpcConnected:
            self.caller.myPopUp2(QMessageBox.Question, 'QMT - rpc device check', self.caller.rpcStatusMess,
                                 QMessageBox.Ok)
        else:
            try:
