...
class TabMain():
    ...
    def displayMNStatus(self, currMN):
        ...
        if statusData is None:
            ...
            if statusData['balance'] is not None:
                self.ui.mnBalance[masternode_alias].setText('&nbsp;<span style="color:purple">%s QMC</span>' % str(statusData['balance']))
    ...
    def onCheckAllMN(self):
        if not self.caller.rpcConnected:
            self.caller.myPopUp2(QMessageBox.Critical, 'QMT - hw device check', "RPC server must be connected to perform this action.")
    ...
    def onStartAllMN(self):
        ...
        # Check RPC & dongle
        if not self.caller.rpcConnected:
            self.caller.myPopUp2(QMessageBox.Critical, 'QMT - hw/rpc device check', "Connect to RPC server and HW device first")
    ...
    def startMN(self):       
        if self.caller.hwStatus != 2:
            self.caller.myPopUp2(QMessageBox.Question, 'QMT - hw device check', self.caller.hwStatusMess, QMessageBox.Ok)
        elif not self.caller.rpcConnected:
            self.caller.myPopUp2(QMessageBox.Question, 'QMT - rpc device check', self.caller.rpcStatusMess, QMessageBox.Ok)
        else:           
            try:
