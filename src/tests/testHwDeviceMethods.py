...
class TestHwDeviceMethods(unittest.TestCase):
    def setUp(self):
        ...
        if hwStatus != 2:
            self.skipTest("Ledger not connected or qmc app closed")
        if not rpcStatus:
    ...
    def test_transaction(self):
        ...
        qmc_address_to = input_data['address_from']
        fee = input_data['fee']
        ...
        print("---------------------------------")
        txraw, amount = self.signTx(self.device, path, utxos, qmc_address_to, fee, rawtransactions)      
    ...
    def test_signature(self):
        ...
        path = input_data['path']
        qmc_address = self.device.chip.getWalletPublicKey(path).get('address')[12:-2]
        ...
        result = self.rpcClient.verifyMessage(qmc_address, signature, message)
        print("sig = %s\naddress=%s" % (signature, qmc_address))
        self.assertTrue(result)
