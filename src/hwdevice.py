...
from btchip.btchip import BTChipException, btchip, getDongle
...
class HWdevice(QObject):
    ...
    def getStatus(self):
        messages = {
            ...
            1: 'Unable to connect to the device. Please check that the QMC app on the device is open, and try again.',
    ...
    def prepare_transfer_tx(self, caller, bip32_path, utxos_to_spend, dest_address, tx_fee, rawtransactions,
        ...
        self.messageText += "<p>Net amount:<br><b>%s</b> QMC</p>" % str(round(self.amount / 1e8, 8))
        if useSwiftX:
            self.messageText += "<p>Fees (SwiftX flat rate):<br><b>%s</b> QMC<p>" % str(round(int(tx_fee) / 1e8, 8))
        else:
            self.messageText += "<p>Fees:<br><b>%s</b> QMC<p>" % str(round(int(tx_fee) / 1e8, 8))
