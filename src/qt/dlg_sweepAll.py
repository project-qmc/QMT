    ...
    def onButtonSend(self):
        try:
            ...
            if not self.main_tab.caller.rpcConnected or self.main_tab.caller.hwStatus != 2:
                self.main_tab.caller.myPopUp2(QMessageBox.Critical, 'QMT - hw/rpc device check',
                                              "Connect to RPC server and HW device first")
                return None

            # Check destination Address      
            if not checkQmcAddr(self.dest_addr):
                self.main_tab.caller.myPopUp2(QMessageBox.Critical, 'QMT - QMC address check',
            ...
            if len(self.rawtransactions) > 0:
                ...
                self.main_tab.caller.hwdevice.prepare_transfer_tx_bulk(self.main_tab.caller, self.rewards,
                                                                       self.dest_addr, self.currFee,
                                                                       self.rawtransactions, self.useSwiftX())
            else:
            ...
            try:
                ...
                if len(tx_hex) > 90000:
                    ...
                    if reply == QMessageBox.Yes:
                        ...
                        self.main_tab.caller.myPopUp2(QMessageBox.Information, 'Transaction NOT sent',
