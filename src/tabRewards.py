...
from PyQt5.Qt import QTableWidgetItem, QHeaderView, QApplication
...
class TabRewards:
            ...
            if len(self.rewards) > 1:  # (collateral is a reward)
                ...
                if not self.caller.rpcConnected:
                    ...
                    if self.apiConnected:
                        self.ui.rewardsList.statusLabel.setText(
                            '<b style="color:red">Found no Rewards for %s</b>' % self.curr_addr)
                    else:
                        self.ui.rewardsList.statusLabel.setText(
                            '<b style="color:purple">Unable to connect to API provider</b>')
    ...
    def onSendRewards(self):
        ...
        if not checkQmcAddr(self.dest_addr):
            self.caller.myPopUp2(QMessageBox.Critical, 'QMT - QMC address check',
